...
function nutrientColor(ac) {
  const n = ac.analysis?.nitrogen_status || 'ok'
...
const LAYER_CONFIG = {
  ndvi:      { offset: 0.05, color: '#22c55e', colorFn: sub => ndviColor(sub.ndvi),         label: 'NDVI' },
  moisture:  { offset: 0.05, color: '#38bdf8', colorFn: sub => moistureColor(sub.moisture), label: 'MOISTURE' },
  nutrients: { offset: 0.05, color: '#f59e0b', colorFn: sub => nutrientColor(sub),          label: 'NUTRIENTS' },
  pipes:     { offset: 0.05, color: '#a78bfa', colorFn: sub => pipeColor(sub),              label: 'IRRIGATION' },
}

// ─────────────────────────────────────────────────────────────────────────────
// TERRAIN ANCHORED CELL (Layer Manager)
// ─────────────────────────────────────────────────────────────────────────────
function TerrainAnchoredCell({ x, z, w, h, color, opacity, isSelected, isHovered, onHover, onClick, heightmapUrl, elevationOffset }) {
  const [heightTex] = useTexture([heightmapUrl])
  const meshRef = useRef()
  const glowRef = useRef()

  const tex = useMemo(() => {
    const u = (x + TW / 2) / TW
    const v = 1 - (z + h + TH / 2) / TH
    const t = heightTex.clone()
    t.needsUpdate = true
  ...
  useFrame(({ clock }) => {
    ...
    const targetOp = isSelected ? Math.max(opacity, 0.8) : isHovered ? Math.max(opacity, 0.6) : opacity
    mat.opacity += (targetOp - mat.opacity) * 0.1

    if (glowRef.current) {
      const pulse = 0.4 + Math.sin(clock.getElapsedTime() * 6) * 0.2
      glowRef.current.material.opacity = isSelected ? pulse : (isHovered ? 0.3 : 0)
    }
  })

  return (
    <group position={[x + w/2, elevationOffset, z + h/2]}>
      <mesh
        ref={meshRef}
      ...
      >
        <planeGeometry args={[w * 0.94, h * 0.94, 16, 16]} />
      ...
      <mesh ref={glowRef} rotation={[-Math.PI / 2, 0, 0]}>
        <planeGeometry args={[w * 1.05, h * 1.05, 16, 16]} />
        <meshBasicMaterial
          color={color}
          transparent
          opacity={0}
...
// ─────────────────────────────────────────────────────────────────────────────
function FieldGrid({ onCellClick, grid, activeLayer, heightmapUrl }) {
  const [hovered, setHovered] = useState(null)
  const activeConf = LAYER_CONFIG[activeLayer] || LAYER_CONFIG.ndvi
    ...
    <group>
      {grid.map((row, ri) =>
        row.map((ac, ci) => {
          ...
          const col = activeConf.colorFn(ac)
          ...
          return (
            <group key={ac.id}>
              <TerrainAnchoredCell
                ...
                color={col}
                opacity={0.15}
                elevationOffset={activeConf.offset}
                heightmapUrl={heightmapUrl}
                isSelected={false}
                isHovered={isHov}
                onHover={() => setHovered(isHov ? null : ac.id)}
                onClick={() => onCellClick(ac)}
              />
              {isHov && (
                <Html
                  position={[x + CW / 2, activeConf.offset + 0.5, z + CH / 2]}
                  center distanceFactor={10} zIndexRange={[60, 0]}
                  ...
                  <div style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 5 }}>
                      ...
                      <span style={{ color: col, fontSize: 10, fontWeight: 900 }}>
                        {ac.id.toUpperCase()} · {ac.analysis?.health_status?.toUpperCase() || 'UNKNOWN'}
                      </span>
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '3px 10px' }}>
                      {[
                        ...
                        { l: 'Yield', v: `${ac.analysis?.yield_qtl_acre || 0}q`,      c: '#a78bfa' },
  ...
  return (
    <group>
      {quads.map(([qx, qz, qw, qh], i) => (
        <mesh key={i} ref={refs[i]} position={[qx, Y, qz]} rotation={[-Math.PI / 2, 0, 0]} renderOrder={3}>
          <planeGeometry args={[qw, qh]} />
          <meshBasicMaterial color="#000c1a" transparent opacity={0} depthWrite={false} depthTest={false} side={THREE.DoubleSide} />
        </mesh>
      ))}
    </group>
  )
...
function CameraAnimator({ target, controlsRef }) {
  ...
  useEffect(() => {
    ...
    if (target.pos) {
      ...
      const currentDir = new THREE.Vector3().subVectors(camera.position, a.startLook).normalize()
      if (currentDir.lengthSq() < 0.1 || currentDir.y < 0.2) {
         currentDir.set(0, 0.8, 0.6).normalize()
      }
      a.endPos.copy(a.endLook).add(currentDir.multiplyScalar(target.distance))
    }
  ...
  useFrame((_, delta) => {
    const a = anim.current
    if (!a.active) return
    a.t = Math.min(1, a.t + delta / 1.5) // Smooth 1.5s transition
...
// ─────────────────────────────────────────────────────────────────────────────
function InnerScene({ satelliteUrl, heightmapUrl, viewMode, activeLayer, selectedL1, onCellSelect, onMicroHover, onMicroSelect, microHoveredId, microSelectedId, grid }) {
  ...
  const cameraTarget = useMemo(() => {
    ...
    return {
      pos: null,
      look: [region.x + region.w / 2, 0.0, region.z + region.h / 2],
      distance: 5.5
    }
  }, [region, viewMode])
  ...
  return (
    <>
      ...
      {viewMode === 'field' && (
        <FieldGrid onCellClick={onCellSelect} grid={grid} activeLayer={activeLayer} heightmapUrl={heightmapUrl} />
      )}
      ...
      {viewMode === 'detail' && region && (
        <>
          ...
          <group>
            {subCells.map(sub => {
              const wx = region.x + sub.sc * (region.w / 3)
              const wz = region.z + sub.sr * (region.h / 3)
              return (
                <TerrainAnchoredCell
                  key={`${activeLayer}-${sub.id}`}
                  x={wx} z={wz} w={region.w/3} h={region.h/3}
                  color={activeConf.colorFn(sub)}
                  opacity={0.65}
                  elevationOffset={activeConf.offset}
                  heightmapUrl={heightmapUrl}
                  isSelected={microSelectedId === sub.id}
                  isHovered={microHoveredId === sub.id}
                  onHover={() => onMicroHover(sub)}
                  onClick={() => onMicroSelect(sub)}
                />
              )
            })}
          </group>
    ...
    <div style={{ width: '100%', height: '100%', position: 'relative', background: '#030d1a' }}>
      ...
      {viewMode === 'detail' && (
        <div style={{
          ...
          {Object.entries(LAYER_CONFIG).map(([id, conf]) => {
            ...
            return (
              <button
                ...
                style={{
                  background: isActive ? `${conf.color}20` : 'transparent',
                  border: `1px solid ${isActive ? conf.color : 'transparent'}`,

...
export interface FieldSceneProps {
  ...
  onCellSelect?: (ac: import('@/lib/fieldDataEngine').AnalyzedCell, row: number, col: number) => void
}
...
function Scene({
  ...
  showIrrigation = false, onCellSelect,
}: SceneProps & {
  focusedZonePos?: { x: number, z: number } | null
  selectedQuadrant?: QuadrantId | null
  onQuadrantSelect?: (id: QuadrantId | null) => void
  ...
  return (
    <>
      ...
      {/* Soil overlay — uses same heightmap geometry as terrain for true anchoring */}
      {activeLayer === 'soil' && (
        <mesh position={[0, 0.02, 0]} rotation={[-Math.PI / 2, 0, 0]} renderOrder={2}>
          ...
          <meshStandardMaterial
            ...
            emissive="#7a3c0a" emissiveIntensity={0.15}
          />
        </mesh>
      )}
      ...
      {/* Grid × Terrain integration — layer-aware, terrain-anchored */}
      <GridTerrainOverlay activeLayer={activeLayer} onCellSelect={onCellSelect} />

...
// ── Constants ─────────────────────────────────────────────────────────────
const PIPE_Y         = -0.15   // render pipes slightly below surface (visible cut-away feel)
const SURFACE_Y      = 1.72    // just above terrain + grid overlay
const CELL_SIZE      = 1.0
...
function MoistureCellMesh({ cell, isHovered, isSelected, onHover, onLeave, onClick }: {
  ...
  const borderPts = useMemo(() => {
    ...
    return [
      new THREE.Vector3(x0, SURFACE_Y + 0.005, z0),
      new THREE.Vector3(x1, SURFACE_Y + 0.005, z0),
      new THREE.Vector3(x1, SURFACE_Y + 0.005, z1),
      new THREE.Vector3(x0, SURFACE_Y + 0.005, z1),
      new THREE.Vector3(x0, SURFACE_Y + 0.005, z0),
    ]
  }, [cell.x, cell.z])
  ...
  return (
    <group>
      ...
      <mesh
        ref={meshRef}
        position={[cell.x, SURFACE_Y, cell.z]}
      ...
      {(isHovered || isSelected) && (
        <Html
          position={[cell.x, SURFACE_Y + 0.3, cell.z]}
          center
          distanceFactor={10}
          zIndexRange={[80, 0]}
...
function PipeMesh({ seg, isHovered, isSelected, onHover, onLeave, onClick }: {
  ...
  const baseOpacity = isSelected ? 0.95 : isHovered ? 0.85 : 0.72

  // Animated pulsing flow on active pipes
  useFrame(({ clock }) => {
    if (!meshRef.current) return
    const mat = meshRef.current.material as THREE.MeshBasicMaterial
    if (seg.status === 'active' && seg.flowLevel !== 'none') {
      const t = clock.getElapsedTime()
      mat.opacity = baseOpacity + Math.sin(t * 3 + seg.from.x) * 0.08
    } else if (seg.blocked) {
      // Slow red throb
      const t = clock.getElapsedTime()
      mat.opacity = 0.6 + Math.sin(t * 1.5) * 0.25
    } else {
      mat.opacity += (baseOpacity - mat.opacity) * 0.1
    }
  })

  // Tube geometry from two nodes
  const curve = useMemo(() =>
    new THREE.CatmullRomCurve3([
      new THREE.Vector3(seg.from.x, PIPE_Y, seg.from.z),
      new THREE.Vector3(seg.to.x,   PIPE_Y, seg.to.z),
    ]),
  [seg.from.x, seg.from.z, seg.to.x, seg.to.z])

  // Click hit box — invisible wide cylinder along the pipe
  ...
  return (
    <group>
      ...
      <mesh
        ...
        onPointerLeave={onLeave}
      >
        <tubeGeometry args={[hitCurve, 8, radius, 7, false]} />
        <meshBasicMaterial
          color={color}
          transparent
          opacity={baseOpacity}
      ...
      {(isHovered || isSelected) && (
        <Html
          position={[(seg.from.x + seg.to.x) / 2, PIPE_Y + 0.35, (seg.from.z + seg.to.z) / 2]}
          center
          distanceFactor={10}
          zIndexRange={[90, 0]}
          style={{ pointerEvents: 'none' }}
...
function FlowParticles({ seg }: { seg: PipeSegment }) {
  ...
  useFrame(({ clock }) => {
    ...
    refs.current.forEach((mesh, i) => {
      ...
      const fromV  = new THREE.Vector3(seg.from.x, PIPE_Y, seg.from.z)
      const toV    = new THREE.Vector3(seg.to.x,   PIPE_Y, seg.to.z)
...
function TrenchLine({ seg }: { seg: PipeSegment }) {
  const color  = PIPE_COLOR[seg.status] ?? '#38bdf8'
  const pts    = useMemo(() => [
    new THREE.Vector3(seg.from.x, PIPE_Y + 0.001, seg.from.z),
    new THREE.Vector3(seg.to.x,   PIPE_Y + 0.001, seg.to.z),
  ], [seg.from.x, seg.from.z, seg.to.x, seg.to.z])
...
function DepthGuides({ seg }: { seg: PipeSegment }) {
  ...
  const color = PIPE_COLOR[seg.status] ?? '#38bdf8'
  const pts   = useMemo(() => [
    new THREE.Vector3(midX, SURFACE_Y, midZ),
    new THREE.Vector3(midX, PIPE_Y,    midZ),
  ], [midX, midZ])
...
function CellInspectorHUD({
  ...
  return (
    <Html
      position={[cell.x + 1.2, SURFACE_Y + 0.8, cell.z]}
      distanceFactor={12}
      zIndexRange={[100, 0]}
      style={{ pointerEvents: 'none' }}
    >
...
function NetworkLegend({ network }: { network: IrrigationNetwork }) {
  return (
    <Html
      position={[-4.8, SURFACE_Y + 1.2, -4.8]}
      distanceFactor={14}
      zIndexRange={[50, 0]}
      style={{ pointerEvents: 'none' }}
    >

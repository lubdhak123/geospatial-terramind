...
import { DiseasePanelCompact } from '@/components/DiseasePanel'
...
function PanelContent({ cell }: { cell: AnalyzedCell }) {
  ...
  return (
    <div style={{ overflowY: 'auto', maxHeight: 'calc(100vh - 120px)', paddingRight: 2 }}>
      ...
      {/* 3. Disease */}
      <div style={{
        background: `${RISK_COLOR[a.disease_risk]}08`,
        border: `1px solid ${RISK_COLOR[a.disease_risk]}20`,
        borderRadius: 8, padding: '8px 10px', marginBottom: 6,
      }}>
        <div style={{ color: RISK_COLOR[a.disease_risk], fontSize: 8, fontWeight: 900, letterSpacing: '0.12em', marginBottom: 6 }}>
          🦠 DISEASE DETECTION
        </div>
        <DiseasePanelCompact result={detectDiseaseFromCell(cell)} />
      </div>
